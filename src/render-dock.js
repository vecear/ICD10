  ...
  function mount(host, ctx) {
    ...
    function restoreFromPip() {
      ...
      /* 解除置頂後，主視窗寬到足以跑工作台就切回工作台。
         ...
         為什麼不是「把視窗縮小」：網頁改不了自己所在視窗的大小。實測（2026-09-01）——
         一般分頁呼叫 resizeTo(340,900) 後 innerWidth／clientWidth 1266 → 1266 完全沒動；
         同一行在 window.open 開出來的視窗上則是 1266 → 326，真的縮了。所以限制是
         「非腳本開的視窗不給改」，不是 API 壞掉。

         那麼側欄回到一個最大化的視窗時，畫面就只能是「一條窄欄＋一大片空白」——
         那既不是側掛也不是工作台。使用者裁示：這種情況直接回工作台。

         門檻用 LAYOUT_MIN_WIDTH 而不是自己挑一個數字：低於它的話 resolveLayout() 會把
         wide 降級成手機版面，那比空白更糟。視窗本來就窄（Ctrl+Alt+D 貼齊的 340px、
         或使用者自己拉窄的）就維持側掛——那正是這個版面該用的場合。

         這行要放在最後：setLayout 會同步觸發 mount() → teardown() → closePip()，
         再繞回這個函式；那時 pipWin 已是 null 會直接 return，不會遞迴。 */
      if (stillMounted && root.innerWidth >= R.LAYOUT_MIN_WIDTH) ctx.store.setLayout('wide');
    }
