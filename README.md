...
- 條文下方的「補充說明」收合著，點開是例外、依據與容易誤讀的地方。**數字看起來衝突時先點開它**
  ...
  （後者是 722 法則的居家血壓）。
- 條文與補充都**一條一行**：依句號與分號斷段（括號內的分號不切），第二條起掛項目符號並
  懸掛縮排，換行的續行才不會跟「下一條」長得一樣。行首的短標（「起始門檻：」「篩檢：」）
  另外標重，因為那正是掃視時要找的詞。斷段只切不改字——`splitSentences` 的段落接回去
  必須逐字等於資料檔原文，E2E 直接拿 `textContent` 比對 `chronic_care.json`。
