...
算出 ASCVD 風險分級、兩張表各自的起始門檻，以及本例達不達標。

**為什麼需要它**：LIPID 的給付判定有三個機械步驟特別容易錯，而三個都不需要臨床判斷——
用錯表（2026-09-01 起表一／表二依健保代碼分流）、極高／非常高風險的「組合」條件接錯
（是「冠狀動脈疾病**合併**什麼」，不是單一條件）、新舊兩表的風險因子定義混用
（新制 6 項含代謝症候群、HDL-C 女性 < 50；舊表 5 項含「或停經者」、HDL-C 男女同為 < 40）。

**兩張表一律都算**。同一位病人可能表一未達、表二卻符合（或反之），差別只在你要開的
品項走哪一張表——結論不同時面板會明講。判定邏輯是 `logic.js` 的純函式 `lipidCoverage()`，
單元測試在 `tests/logic.test.mjs`（逐級門檻、極高風險的組合條件、新舊風險因子定義的三處差異、
